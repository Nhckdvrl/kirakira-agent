...
class _FakePipeline:
    ...
    async def run(self, msg, key, *, dispatch_outbound=True):
        assert dispatch_outbound is False, "控制面 turn 不能再走 bus 出站"
        self.seen_metadata = dict(msg.metadata)
...
async def test_binding_projects_attention_and_tool_calls():
    ...
    assert pipeline.seen_metadata["session_key_override"] == "programmatic:x"
