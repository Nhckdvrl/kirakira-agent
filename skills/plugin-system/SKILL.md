...
kirakira 的插件模型比 Akashic 简单，**没有 marketplace、没有全局缓存、没有热重载**：
...
- **改动都要重启才生效**：安装、启用、禁用、卸载后都必须重启 kirakira-agent；运行中不会热重载。
- 不要查找或创建 `~/.akashic-plugin/`、`manifest.yaml`、`registry.json`、`plugin.json` 或 `mcp/servers.json`。
...
安装成功后工具会提示需要重启。重启后用 `plugin_list` 确认已加载、`plugin_doctor <name>` 校验结构。
...
三者都只改磁盘状态，**必须重启**才真正生效。卸载保留 `plugin-data`，不要手动删数据、配置、token 或数据库。

## 完成判定

工具返回成功只代表磁盘状态已改，不代表能力已生效。汇报完成前必须：
