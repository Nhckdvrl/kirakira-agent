...
判断顺序（`loop.py:_tick`）：**alert 优先直推** → 有新 content 且不在冷却期则做**兴趣判断** →
都没推 → 交给 **Drift**。`context` 从不单独触发，只作为本轮判断的背景注入 prompt。
...
`PROACTIVE_CONTEXT.md` 规则 + 当前 context + 候选事件。
...
对应测试：`tests/test_proactive.py`（11 项）。
