...
def _reference_alignment(root: Path) -> dict[str, Any]:
    ...
    matched: list[str] = []
    drifted: list[str] = []
    for local in sorted(local_dir.glob("*.py")):
        reference = reference_dir / local.name
...
def doctor(workspace: Path, *, project_root: Path | None = None) -> dict[str, Any]:
    ...
    reference: dict[str, Any] = {"pin": REFERENCE_PIN, "exists": reference_dir.exists()}
    if reference_dir.exists():
        try:
            ...
            reference.update({"head": head, "matches_pin": head == REFERENCE_PIN})
    ...
    report = {
        ...
        "modules": modules,
        "reference": reference,
        "source_alignment": _reference_alignment(root),
