...
from kirakira_agent.runtime import DefaultReasoner
...
class SubagentManager:
    ...
    async def spawn(
        ...
        if mode == "background":
            if len(self._task_by_id) >= _MAX_BACKGROUND_JOBS:
                return "Error: already running %d background subagents (limit %d)" % (
